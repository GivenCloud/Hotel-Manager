...
// Function to fetch services assigned to a specific hotel
const fetchAssignedServices = async (hotelId: string) => {
  try {
    const response = await apiClient.get(`http://hotel-manager.test/api/hotel/${hotelId}/services`, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem('token')}` // Include the token in the header
      }
    });
    ...
    console.error(`Error fetching assigned services for hotel ${hotelId}:`, error);
    return [];
  }
};

// Function to assign a service to a hotel
const assignService = async (service: Service | null) => {
  if (!service) return;

  // Check if the service is already assigned
  if (assignedServices.value.some((s: Service) => s.id === service.id)) {
  ...
  try {
    await apiClient.post(`http://hotel-manager.test/api/hotel/${hotelId}/services`, {
      hotel_id: hotelId,
...
const removeService = async (service: Service) => {
  try {
    await apiClient.delete(`http://hotel-manager.test/api/hotel/${hotelId}/services`, {
      data: {
        hotel_id: hotelId,
        service_id: [service.id],
      },
    ...
    assignedServices.value = assignedServices.value.filter((s: Service) => s.id !== service.id);
  } catch (error) {
...
onMounted(async () => {
  ...
  assignedServices.value = await fetchAssignedServices(hotelId);
});
</script>
